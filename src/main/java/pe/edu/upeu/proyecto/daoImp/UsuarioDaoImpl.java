...
public class UsuarioDaoImpl implements UsuarioDao {
	...
	public List<Map<String, Object>> getAllUsers() {
		...
		simpleJdbcCall = new SimpleJdbcCall(jdbcTemplate).withCatalogName("PKG_USUARIOS")
				.withProcedureName("SP_LIS_USUARIOS")
	...
	public List<Usuario> getAll() {
		return jdbcTemplate.query("Select * from usuarios", BeanPropertyRowMapper.newInstance(Usuario.class));
	}
	@Override
	public int regUsuario(String clave, Persona persona) {
		...
		if (read(persona.getCorreo()) == null) {
			response = jdbcTemplate.update("call PKG_USUARIOS.SP_REG_USUARIOSOL(?,?,?,?,?,?,?,?,?,?)",
					persona.getId_tipodoc(), persona.getNombres(), persona.getApepat(), persona.getApemat(),
					persona.getNumero_doc(), persona.getFec_nac(), persona.getDireccion(), persona.getTelefono(),
					persona.getCorreo(), clave);
			System.out.println("El usuario fue creado");
			System.out.println(response);
		} else {
			System.out.println("El usuario ya está en uso");
		}
		return response;
	}
	@Override
	public int updUsuario(Usuario u) {
		...
		return jdbcTemplate.update("call PKG_USUARIOS.SP_UPD_USUARIO(?,?,?,?,?)", u.getId_usuario(), u.getNombre(),
				u.getUsuario(), u.getClave(), u.getId_rol());
	}
	@Override
	public int create(Usuario u) {

		return jdbcTemplate.update("call PKG_USUARIOS.SP_REG_USUARIO(?,?,?,?)", u.getNombre(), u.getUsuario(),
				u.getClave(), u.getId_rol());
		 
	}
	@Override
	public int activacion(int id) {
		return jdbcTemplate.update("call PKG_USUARIOS.SP_ACT_USUARIO(?)", id);
	}
	@Override
	...
	public Usuario read(String nomusuario) {
		...
		try {
			user = jdbcTemplate.queryForObject(SQL, new Object[] { nomusuario },
	...
	public Usuario validarUsuario(String nomuser) {
		String SQL = "select id_usuario, nombre, usuario, clave, id_rol, correo_activacion, estado from usuarios where usuario=?";
